...
router.post('/', requireAdmin, async (req, res) => {
  ...
  const { rows } = await query(
    `INSERT INTO products (id, name, category_id, price, mrp, stock, description, image)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [id, name.trim(), category, Number(price) || 0, Number(mrp) || Number(price) || 0, Number(stock) || 0, description || '', image || '']
  );
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  const { rows } = await query(
    ...
    [name, category, price, mrp, stock, description, image, req.params.id]
  );
  if (!rows[0]) return res.status(404).json({ error: 'Product not found.' });
  res.json({ product: mapProduct(rows[0]) });
});
